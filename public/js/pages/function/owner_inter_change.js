...
$(function () {
    $('#form_validation').validate({
		...
		submitHandler: function(form) {	
			...
			$.post(SITEURL+"apicontroller/ownership_change",$('#form_validation').serializeArray(),function(data){
				data = data.replace(/^\s+|\s+$/g, "");
				data=JSON.parse(data);
				console.log(data)
				if(data.error)
				{					
					showWithTitleMessage(data.message,'');
				}	
...
$(document).ready(function(){
	...
	$.post(SITEURL + "admin/fetch_serial_list_by_companyId_dealerId", { 'veh_company_id': value, 'dealer_id': dealerId }, function (data) {
		data = data.replace(/^\s+|\s+$/g, "");
		data=JSON.parse(data);
		// Serial Number
...
$('#new_phone').on('keyup',function(){
    if($("#new_phone").val() == $("#veh_owner_phone").val() )
    {
		showWithTitleMessage('Error Found',"Existing Number And New Number are Same");	
        $("#new_phone").val('') = '';
	}
    else{
    var value=$(this).val();
    if(value.length != 10)
    {
        ...
        $("#new_address").val('');
        $("#new_email").val('');
        $("#new_name,#new_address,#new_email").trigger("change");
        return true;
    }else{
        $.post(SITEURL+"apicontroller/fetch_customer_by_phone",{'phone':value},function(data){
            data = data.replace(/^\s+|\s+$/g, "");
            data=JSON.parse(data);

            if(data.customer){
                $("#new_owner_id").val(data.customer.c_customer_id);
                $("#new_name").val(data.customer.c_customer_name);
                $("#new_address").val(data.customer.c_address);
                $("#new_email").val(data.customer.c_email);
            }else{
                $("#new_owner_id").val('');
                $("#new_name").val('');
                $("#new_address").val('');
            ...
            $("#new_name,#new_address,#new_owner_id").trigger("change");
