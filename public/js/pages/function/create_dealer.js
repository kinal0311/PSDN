var fileInput = document.getElementById('phone');
fileInput.addEventListener('change', phoneNumberValidation);
...
function phoneNumberValidation(e, th) {
	var fileInputnew = document.getElementById('veh_owner_phone');
...
function dealer_photo_upload(e,th){
		...
		formData.append('upload_profile_photo', $('input[type=file]')[5].files[0]);
		$.ajax({
				type: "POST",
...
$(function () {
    ...
    $('#form_validation').validate({
		...
		submitHandler: function(form) {	
			...
			$.post(SITEURL + "admin/create_new_dealer_records", $('#form_validation').serializeArray(), function (data) {
...
$(document).ready(function(){
	...
	$('[name=user_typesd]').on('change',function(){
		var value=$(this).val();
		...
		var user_company_id=$('#user_company_id').val();
		$.post(SITEURL + "admin/fetch_list_of_users", { 'user_type': 2, 'user_company_id': user_company_id }, function (data) {
			data = data.replace(/^\s+|\s+$/g, "");
			data=JSON.parse(data);
