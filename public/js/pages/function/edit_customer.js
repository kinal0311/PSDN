...
$(function () {
	...
	$('#form_validation').validate({
		...
		submitHandler: function (form) {
			$.post(SITEURL+"admin/count_customer_records",$('#form_validation').serializeArray(),function(data){
				...
				if(data.count !== 0)
					{
						swal({
							...
							html: true,
							text: 'Customer detail will change to all the Certificate info',						
						}, function (isConfirm) {
						if(isConfirm)
						{
							$.post(SITEURL + "admin/update_customer_records", $('#form_validation').serializeArray(), function (data) {
								data = data.replace(/^\s+|\s+$/g, "");
								data = JSON.parse(data);
								if (data.error) {
