...
class Invoicemodel extends CI_Model {
   ...
   public function getcompanyalldetails_fromusertype($param){
		...
		public function fetch_customerdropdowndatas($param){
			
			
			
			$query = $this->db->query(" select * from ci_customers where c_customer_id = '".$param."'  ");
			$row = $query->row();
		...
		public function fetch_companydropdowndatas($param){
			...
			$query = $this->db->query(" select t1.*, t2.* from ci_users t1 inner join ci_company t2 on t1.user_company_id = t2.c_company_id where t1.user_id = '".$param."'  ");
			$row = $query->row();
		...
		public function getall_entries_proforma_invoice($user_company_id,$user_type,$user_id){
           // print_r($user_id);
			...
			if($user_type=="4"){
				...
				$subquery .= " AND t1.user_id = '".$user_id."' ";
			}

			$query = " select t1.*, t2.*, datediff(now(),t1.duedate) as statuscheck, t3.* from ci_proformainvoice t1 left join ci_company t2 on t1.company_id = t2.c_company_id left join ci_users t3 on t3.user_id = t1.user_id ".$subquery." ";
           // print_r($query);exit;
			$statedatas = $this->db->query($query)->result();
		...
		public function fetch_proformainvoicedatas($param){
			...
			$query = $this->db->query(" select t1.*,t2.user_own_company, t1.gstin,t2.user_info, t2.user_email, t3.* from ci_proformainvoice t1 left join ci_users t2 on t2.user_id = t1.user_id left join ci_company t3 on t3.c_company_id = t1.company_id ".$subquery);
			$row = $query->row();
