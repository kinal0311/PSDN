...
<?php if(check_permission($user_type,'menu_product_create')){ ?>
                            ...
                            <!--            <li><a href="<?php echo base_url().'admin/create_product'; ?>">Create</a></li>                                        -->
                            <!--        </ul>-->
                            <!--    </li>-->
                            <!--</ul>  -->
 <?php } ?>                                           
					...
					<!--- Search---->
								<form action="<?php echo base_url() ?>admin/product_list" name="searchfilter"  id="searchfilter" method="get" />				
										<input type="hidden" name="offset"   id="offset" value="<?php echo isset($_GET['offset'])?$_GET['offset']:0; ?>">				
										<div class="row clearfix">
											<div class="col-sm-4">
												<div class="form-group">
													<div class="form-line">
														<input type="text" class="form-control" placeholder="Search by product name" id="search" value="<?php echo isset($_GET['search'])?$_GET['search']:""; ?>" name="search">
													</div>
												</div>
											</div>
			...
			<!---- Search ---->
              ...
              <table id="mytable" class="table table-bordred table-striped">
                   ...
                   <thead>
					   ...
					   <th>TAC Number</th>
					   <th>Unit Price</th>   
<?php if(check_permission($user_type,'product_edit')){ ?>
					   <th>Action</th>                     
<?php } ?>				  
                   </thead>
					<tbody>  
				...
				if(count($listofProducts)>0)
				{
						...
						{							
							$enc_product_Id = base64_encode($value['p_product_id']);
						?>
						<tr>
						<td><?php echo $sno; ?></td>
						<td><?php echo $value['c_company_name']; ?></td>
						<td><?php echo $value['p_product_name']; ?></td>
						<td><?php echo $value['p_tac_no']; ?> </td>
						<td><?php echo $value['p_unit_price']; ?></td>
<?php if(check_permission($user_type,'product_edit')){ ?>
						<td class="text-left">
							 <a class='btn btn-info btn-xs' href="<?php echo base_url()."admin/edit_product/?q=".$enc_product_Id; ?>">
	...
	<script>	
		$(function() {
				$('#pageformat').pagination({
					items: '<?php echo $totalNoOfProducts; ?>',
