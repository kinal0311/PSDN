                            ...
                            <ul class="header-dropdown m-r--5">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="<?php echo base_url().'admin/create_company'; ?>">Create Manufacturer</a></li>                                        
                                    </ul>
                                </li>
                            </ul>  
<?php } ?>                                           
					...
					<!--- Search---->
								<form action="<?php echo base_url() ?>admin/company_list" name="searchfilter"  id="searchfilter" method="get" />				
										<input type="hidden" name="offset"   id="offset" value="<?php echo isset($_GET['offset'])?$_GET['offset']:0; ?>">				
										<div class="row clearfix">
											<div class="col-sm-4">
												<div class="form-group">
													<div class="form-line">
														<input type="text" class="form-control" placeholder="Search by Company Name, TAC Number" id="search" value="<?php echo isset($_GET['search'])?$_GET['search']:""; ?>" name="search">
													</div>
												</div>
											</div>									
			...
			<!---- Search ---->
              ...
              <table id="mytable" class="table table-bordred table-striped">
                   ...
                   <thead>
					   ...
					   <th>TAC Number</th>                     
					   <th>COP Validity</th>					
<?php if(check_permission($user_type,'inventry_edit')){ ?>
					   <th>Action</th>                      
<?php } ?>		  
                   </thead>
					<tbody>  
						<?php
				if(count($listofCommpanys)>0)
				{
						...
						foreach($listofCommpanys as $key=>$value)
						{							
							$enc_com_Id = base64_encode($value['c_company_id']);
						?>
						<tr>
						<td><?php echo $sno; ?></td>
						<td><?php echo $value['c_company_name']; ?></td>
						<td><?php echo $value['c_tac_no']; ?></td>
						<td><?php echo date('Y-m-d',strtotime($value['c_cop_validity'])); ?></td>
<?php if(check_permission($user_type,'inventry_edit')){ ?>
						<td class="text-left">
							 <a class='btn btn-info btn-xs' href="<?php echo base_url()."admin/edit_company/?q=".$enc_com_Id; ?>">
	...
	<script>	
		$(function() {
				$('#pageformat').pagination({
					items: '<?php echo $totalNoOfCompanys; ?>',
