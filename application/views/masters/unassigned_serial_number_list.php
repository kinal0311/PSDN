        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<!--- Search---->
								<form action="<?php echo base_url() ?>admin/unassigned_serial_number_list" name="searchfilter"  id="searchfilter" method="get" />				
										<input type="hidden" name="offset"   id="offset" value="<?php echo isset($_GET['offset'])?$_GET['offset']:0; ?>">				
										<div class="row clearfix">
											<div class="col-sm-3">
												<div class="form-group">
													<div class="form-line">
														<input type="text" class="form-control" placeholder="Search by IMEI, Serial No." id="search" value="<?php echo isset($_GET['search'])?$_GET['search']:""; ?>" name="search">
													</div>
												</div>
											</div>									
											 <!-- <div class="col-sm-3">
                                        <div class="form-group form-float">
                                            <div class="form-line">
                                               <select class="form-control show-tick" name="company_id"
                                                    id="company_id" data-live-search="true">
                                                    ...
                                                    <?php
                                                       ...
                                                       ?>
                                                               <option <?php echo $selected; ?>
                                                                   value="<?php echo $value['c_company_id'] ?>">
                                    ...
                                    <input type="hidden" id="hid_product_id" value="1">
                                    <input type="hidden" id="company_id" value="2">
                                    <input type="hidden" id="s_product_id" value="1">
                                    <!-- <div class="col-sm-3">
                                        <div class="form-group form-float">
                                            <div class="form-line">
                                                <?php 
                                                   $product_id = preg_replace("/&#?[a-z0-9]+;/i","",$_REQUEST["s_product_id"]);
                                                   ?>
                                                <input type="hidden" id="hid_product_id"
                                                    value="<?php echo $product_id; ?>">
                                                <select class="form-control show-tick" name="s_product_id"
                                                    id="s_product_id" data-live-search="true">
                                                    ...
                                                    <?php
                                                           ...
                                                           ?>
                                                    <option <?php echo $selected; ?>
                                                        value="<?php echo $value['p_product_id'] ?>">
                                                        <?php echo $value['p_product_name'] ?>
                                                   </option>
                                                        <?php
                                                           }
                                                           ?>
													...
													<div class="form-line">
														<select class="form-control show-tick" name="used_status" id="used_status" data-live-search="true">
															...
															<option <?php echo $unused; ?> value="0">Unused</option>							
															   ...
															   <!-- <?php //echo json_encode($stateList);?> -->
                                        <div class="col-sm-3">
                                            <div class="form-group form-float">
                                                <div class="form-line">
...
<?php } ?>						
						<input type="hidden" id="hid_mode" name="hid_mode" value="unassigned">
						<!--<input type="hidden" id="hid_company_id" name="hid_company_id" value="<?php echo $_GET['company_id']; ?>">-->
						<!--<input type="hidden" id="hid_product_id" name="hid_product_id" value="<?php echo $_GET['s_product_id']; ?>">-->
...
<?php if(check_permission($user_type,'assign_to_distributer_edit')){ ?>
					   <th>Action</th>				   				   
<?php } ?>					   
                   </thead>
					<tbody>  
				...
				if(count($listofSerialNumbers)>0)
				{
						...
						if(isset($_GET['offset']) && (int)$_GET['offset']>0)
						{
				            ...
				            $sno= (((int)$_GET['offset']-1)*(int)$_GET['recs'])+1;
						}
						foreach($listofSerialNumbers as $key=>$value)
						{			
							$enc_serial_id = base64_encode($value['s_serial_id']);				
							$userType='';
						...
						<td><?php echo $sno; ?></td>
<?php if(check_permission($user_type,'assign_to_distributer_assign')){ ?>
						<td><input type="checkbox" class="chk-ind" id="chk<?php echo $sno; ?>" name="serial_ids[]"  value="<?php echo $value['s_serial_id']; ?>" > <label for="chk<?php echo $sno; ?>"></label></td>
<?php } ?>						
						<td><?php echo $value['s_serial_number']; ?></td>
						<td><?php echo $value['s_imei']; ?></td>
						<td><?php echo $value['s_mobile']; ?></td>
						<td><?php echo $value['s_mobile_2']; ?></td>
...
<?php if(check_permission($user_type,'assign_to_distributer_edit')){ ?>
						<td class="text-left">
							 <a class='btn btn-info btn-xs' href="<?php echo base_url()."admin/edit_unassigned_serial_number/?q=".$enc_serial_id; ?>">
	...
	<script>	
		$(function() {
				$('#pageformat').pagination({
				    ...
				    itemsOnPage: '<?php echo (int)$_GET['recs']; ?>',
