...
<style type="text/css">
  ...
  .datetr{background: grey !important;color: white  !important;}
    ...
    <table class="table">
      <tr>
          <th colspan="25" class="bordernone" style="border: none;text-align: center;">Sales Report</th>
      </tr>
    </table>
    <table class="table">
        <tr>
            <th  class="bordernone"  colspan="3">Total Sales : <?php echo count($reportData); ?></th>
            <th  class="bordernone" style="    float: right;">Start Date  : <?php echo date('Y-m-d',strtotime($params['start_date'])); ?></th>
        </tr>
        <?php
        $dealerName='All';
        ...
        <tr> 
            ...
            <th  class="bordernone" style="    float: right;">End Date  : <?php echo date('Y-m-d',strtotime($params['end_date'])); ?></th>    
        </tr>
      ...
      if(count($reportData)>0)
      {
        ...
        foreach ($reportData as $key => $value) {      
         ...
         $currentDate=date('Y-m-d',strtotime($currentDate));   
           if((string)$Available_SLD==='0')
           {
            ?>
              <tr class="datetr"><th colspan="4">Available SLD</th></tr>
            <?php
            $Available_SLD++;
           }  
            if(isset($value['veh_create_date']) && !empty($value['veh_create_date']) && (string)$Solded_SLD==='0')
            {
            ?>
              <tr class="datetr"><th colspan="4">Solded SLD</th></tr>
            <?php
               $Solded_SLD++;
            }       
        ?>      
...
<div style="margin:auto">
 ...
 <script type="text/javascript">
    ...
    var style='<link href="<?php echo base_url(); ?>public/plugins/bootstrap/css/bootstrappdf.css" rel="stylesheet" />';style='';
            ...
            printWindow.document.write('<html><head><title>Sales Report</title>'+style);
