...
// 		echo "<pre>";print_r("_FILES");exit;
        ...
        if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if($_FILES) {
                ...
                if(strlen($name)) {
                    ...
                    $explodedArray = explode(".", $name);
					$lastIndex = end($explodedArray);
                    if(in_array(($lastIndex),$valid_formats)) {

                        if($size<(5242880)) {
                       
                            $actual_image_name = time().".".$lastIndex;
	...
	// Upload Dealer Profile photo
	public function vehicle_owner_id_proof() {	
		...
		$path = "public/upload/vehicle_owner_id_proof";
		if (!is_dir($path)) {
			mkdir($path, 0777, TRUE);
		}
	...
	try {
        if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            if($_FILES) {
                $name = $_FILES['vehicle_owner_id_proof']['name'];
                $size = $_FILES['vehicle_owner_id_proof']['size'];
                if(strlen($name)) {
                    list($txt, $ext) = explode(".", $name);
                    if(in_array(strtolower($ext),$valid_formats)) {
                        if($size<(5242880)) {
                            $actual_image_name = $txt."_".time().".".$ext;
                            $tmp = $_FILES['vehicle_owner_id_proof']['tmp_name'];
                            if(move_uploaded_file($tmp, $path.$actual_image_name))
                            {
                               $returnResponse['success']=true; 
							   $returnResponse['path']=$path.$actual_image_name; 
	...
	// Upload Dealer Profile photo
	public function vehicle_owners_photo_upload() {	
	...
	try {
        if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            if($_FILES) {
                $name = $_FILES['vehicle_owners']['name'];
                $size = $_FILES['vehicle_owners']['size'];
                if(strlen($name)) {
                    list($txt, $ext) = explode(".", $name);
                    if(in_array(strtolower($ext),$valid_formats)) {
                        if($size<(5242880)) {
                            $actual_image_name = time().".".$ext;
                            $tmp = $_FILES['vehicle_owners']['tmp_name'];
                            if(move_uploaded_file($tmp, $path.$actual_image_name))
                            {
                               $returnResponse['success']=true; 
							   $returnResponse['path']=$path.$actual_image_name; 
	...
	// Upload Dealer Profile photo
	public function rc_book_photo_upload() {	
		...
		$path = "public/upload/rc_book_photos";
		if (!is_dir($path)) {
			mkdir($path, 0777, TRUE);
		}
	...
	try {
        if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            if($_FILES) {
                $name = $_FILES['rc_book']['name'];
                $size = $_FILES['rc_book']['size'];
                if(strlen($name)) {
                    list($txt, $ext) = explode(".", $name);
                    if(in_array(strtolower($ext),$valid_formats)) {
                        if($size<(5242880)) {
                            $actual_image_name = time().".".$ext;
                            $tmp = $_FILES['rc_book']['tmp_name'];
                            if(move_uploaded_file($tmp, $path.$actual_image_name))
                            {
                               $returnResponse['success']=true; 
							   $returnResponse['path']=$path.$actual_image_name; 
